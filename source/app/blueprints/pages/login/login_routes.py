    ...
    def oidc_authorise():
        ...
        try:
            if 'id_token' not in access_token_resp:
                log.error('OIDC authentication failed: \'id_token\' not found in access token response')
                track_activity(
            ...
            if not user_login:
                log.error('OIDC authentication failed: username or email not found in id_token')
                track_activity(
